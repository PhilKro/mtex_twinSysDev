...
warning('CHANGE, MORIS ARE NOW SYMMETRISED, HOW DOES IT WORK FOR arrays of moris')
mori = mori.symmetrise;
...
if isempty(gB)
    warning('cleanUpPseudoSym:NoMatchingBoundaries', 'No grain boundaries found with misorientation matching the pseudo-symmetry within the specified tolerance.');
    grainsM = grains;
    return;
end
...
is_target_phase = (reshape(ebsd.phaseId, size(ebsd.grainId)) == pseudoSym_phase_id);
